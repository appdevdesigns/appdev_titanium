var jQuery = module.exports = require('jquery');
  ...
  */  var ajaxHandlerBindings, jsre, r20, rquery, rts, rurl, utils, _fn, _i, _len, _ref;
  ...
  utils.extend(jQuery, ajaxHandlerBindings, {
    ajaxSetup: function(settings) {
      return utils.extend(jQuery.ajaxSettings, settings);
    },
    ajaxSettings: {
      global: true,
      type: "GET",
    ...
    param: function(a) {
      ...
      buildParams = function(prefix, obj) {
        if (jQuery.isArray(obj)) {
          return jQuery.each(obj, function(i, v) {
            var p, x;
            if (traditional || /\[\]$/.test(prefix)) {
              return add(prefix, v);
            } else {
              x = jQuery.isObject(v) || jQuery.isArray(v) ? i : "";
              p = prefix + "[" + x + "]";
    ...
    ajax: function(origSettings) {
      ...
      s = utils.extend({}, jQuery.ajaxSettings, origSettings);
      status = "";
      data = {};
      ...
      trigger = function(type, arg) {
        var obj;
        obj = s.context ? s.context : jQuery;
